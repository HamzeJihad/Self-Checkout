...
const RestaurantCategories = ({ restaurant }: RestaurantCategoriesProps) => {
  ...
  return (
    <div className="relative z-10 mt-[-1.5rem] rounded-t-3xl  bg-white">
      <div className="p-5">
      ...
      <ScrollArea className="w-full">
        <div className="flex w-max space-x-4 p-4 pt-0">
          {restaurant?.menuCategories.map((category) => (
            <Button key={category.id} variant={getCategoryButtonVariant(category)} className="rounded-full" onClick={() => handleCategorySelect(category)}>
              {category.name}
            </Button>
          ))}
        </div>
      ...
      <h3 className="px-4 py-1 text-lg font-semibold">{selectedCategory?.name}</h3>
